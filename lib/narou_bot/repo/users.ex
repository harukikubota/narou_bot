defmodule NarouBot.Repo.Users do
  ...
  def new_user(line_id) do
    User.changeset(%User{}, %{line_id: line_id})
    |> Repo.insert!
  end

  def find_by_line_id(line_id) do
    Repo.get_by(User, line_id: line_id)
  end
  ...
  defp _notification_target_users(:new_post_novel, %{writer_id: writer_id}) do
    from(
      u in User,
        as: :user,
      join: uw in "users_check_writers",
        on: u.id == uw.user_id,
  ...
  defp _notification_target_users(:delete_novel_episode, %{novel_id: novel_id, episode_id: episode_id}) do
    record_status = Status.__enum_map__() |> Keyword.fetch!(:notificated)
